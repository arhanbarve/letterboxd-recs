...
- If Cloudflare blocks the scrape from your hosting provider's IP, set `SCRAPER_PROXY_SERVER` (and `SCRAPER_PROXY_USERNAME`/`SCRAPER_PROXY_PASSWORD` if needed) on the backend to route scraping through a residential proxy.
