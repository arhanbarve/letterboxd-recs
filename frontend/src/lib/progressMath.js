...
// "resolving" is determinate: the film count comes from the stored import, so
// the backend knows the total before it makes a single request (unlike the old
// scrape, which discovered films as it paginated).
const DETERMINATE_STAGES = new Set(["resolving", "enriching", "scoring"]);
...
function bandFor(stage) {
  return STAGE_BANDS[stage] || STAGE_BANDS.resolving;
}
...
export function computePercent(status, { stageElapsedMs = 0 } = {}) {
  ...
  if (stage === "error" || stage === "cancelled" || (!STAGE_BANDS[stage] && stage !== "resolving")) {
    return 0;
  }
  ...
  const estSec = estimateStageDurationSec(stage);
...
const STAGE_ORDER = ["resolving", "enriching", "profiling", "scoring"];
  ...
  for (let i = idx; i < STAGE_ORDER.length; i++) {
    ...
    if (s === stage) {
      if (DETERMINATE_STAGES.has(s) && typeof status.total === "number" && status.total > 0 && status.current > 0) {
        ...
        remaining += rate > 0 ? (status.total - status.current) / rate : estimateStageDurationSec(s);
      } else {
        const est = estimateStageDurationSec(s);
        remaining += Math.max(0, est - stageElapsedMs / 1000);
      }
    } else {
      remaining += FLAT_ESTIMATE_SEC[s] ?? estimateStageDurationSec(s);
    }
  }
  return remaining;
}
